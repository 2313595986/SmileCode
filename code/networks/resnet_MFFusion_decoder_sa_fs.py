...
class Encoder2_MFFusion_Decoder(nn.Module):
    ...
    def __init__(self, num_channels1=1, num_channels2=1, num_classes=2):
        ...
        # self.up5 = Upsample(64, 32, kernel_size=(2, 2, 1), stride=(2, 2, 1))
    ...
    def forward(self, x1, x2, mask=None, fg_prototype=None, bg_prototype=None, stage='train'):
        ...
        if fg_prototype is not None:
            pos_fg_cos = F.interpolate(F.cosine_similarity(fusion_out4[0:1].view(1, 1024, -1), fg_prototype.squeeze(-1).squeeze(-1)).view(1, 5, 5, 25).unsqueeze(1),
                                    [72, 72, 200], mode='trilinear')

        if bg_prototype is not None:
            pos_bg_cos = F.interpolate(F.cosine_similarity(fusion_out4[0:1].view(1, 1024, -1), bg_prototype.squeeze(-1)
                                                           .squeeze(-1)).view(1, 5, 5, 25).unsqueeze(1),
                                       [72, 72, 200], mode='trilinear')
        ...
        if stage == 'train':
            ...
            if mask is not None:
                ...
                fg_p = F.avg_pool3d(fusion_out4[0:mask.size(0)] * mask, (5, 5, 25))
                bg_p = F.avg_pool3d(fusion_out4[0:mask.size(0)] * (1 - mask), (5, 5, 25))

                return [out_cls_fusion, out_seg, fg_p, bg_p]
            else:
                return [out_cls_fusion, out_seg, pos_fg_cos, pos_bg_cos]
        else:
